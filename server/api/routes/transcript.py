...
async def get_txt(session_id: str, request: Request, db: Session = Depends(get_db)) -> Response:
    user = get_optional_user_from_request(request, db)
    return await runtime.get_txt(
        session_id,
...
async def get_jsonl(session_id: str, request: Request, db: Session = Depends(get_db)) -> Response:
    user = get_optional_user_from_request(request, db)
    return await runtime.get_jsonl(
        session_id,
        user_id=user.id if user is not None else None,
        guest_grant_id=read_guest_artifact_grant(request),
    )
...
async def get_zip(session_id: str, request: Request, db: Session = Depends(get_db)) -> Response:
    user = get_optional_user_from_request(request, db)
    return await runtime.get_zip(
        session_id,
        user_id=user.id if user is not None else None,
        guest_grant_id=read_guest_artifact_grant(request),
    )
...
async def get_screenshot(
    ...
    user = get_optional_user_from_request(request, db)
    return await runtime.get_screenshot(
        session_id,
        filename,
        user_id=user.id if user is not None else None,
        guest_grant_id=read_guest_artifact_grant(request),
...
async def get_debug_audio(
    ...
    user = get_optional_user_from_request(request, db)
    return await runtime.get_debug_audio(
        session_id,
        filename,
        user_id=user.id if user is not None else None,
        guest_grant_id=read_guest_artifact_grant(request),
